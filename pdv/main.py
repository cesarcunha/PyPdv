...
from pdv.Database.Controller_db import get_user
...
class pdv(MDApp):
    ...
    def authenticate(self):
        user = self.manager_screens.get_screen("login screen").ids["login_user"].text
        password = self.manager_screens.get_screen("login screen").ids["login_password"].text
        check_user = get_user(data={"username": user})

        if check_user:
            self.manager_screens.current = "sale screen"
            print("Encontrei o cara")
        else:
            print("tenta de novo")

        # if user.upper() == "CESAR" and password.upper() == "MASTER":
        #    self.manager_screens.current = "sale screen"
        # else:
        #    print("Deu ruim!!")
        # print(user,password)
